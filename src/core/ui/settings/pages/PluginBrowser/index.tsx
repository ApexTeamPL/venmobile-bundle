  ...
  return (
    <Card>
      <Stack spacing={16}>
        ...
        >
          <View style={{ flexShrink: 1 }}>
            ...
            <Text variant="text-md/semibold" color="text-muted">
              by {display.authors?.map((a) => a.name).join(", ") || "Unknown"} (
              {version})
            </Text>
          </View>
          <View>
        ...
        <Text variant="text-md/medium">{display.description}</Text>
      </Stack>
    </Card>
  );
}
...
function RepositoryRow(props: { url: string }) {
  ...
  return (
    <TableRow
      label={
        isOfficial ? "Bunny's Repository" : (repo.$meta?.name ?? "Unknown")
      }
      subLabel={props.url}
      trailing={
